...
- [x] `cargo metadata --locked --no-deps` — workspace resolves with the deleted members absent
- [x] `cargo nextest run --workspace --locked` — 100 tests passed
- [x] `cargo test --workspace --doc --locked` — doctests passed
- [x] `cargo clippy --workspace --all-targets --locked -- -D warnings` — clean
- [x] `cargo xtask alpha-check` — 15 AC checks and 33 active BDD scenarios passed
