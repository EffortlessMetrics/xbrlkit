...
Feature: Validation finding construction
  ...
  Scenario: Shared constructors preserve finding field mappings
    ...
    Then the baseline finding serializes with no member or subject
    And the context finding serializes subject "ctx-1" with no member
    And the builder finding serializes member "us-gaap:ScenarioAxis" and subject "us-gaap:ActualMember"
